...
def _assess_quality_fallback(page: pymupdf.Page, text: str) -> Dict[str, str]:
    ...
    return {
        ...
        "doc_quality_issues": "document seems fine" if doc_quality == "Good" else "blur or low resolution",
    }
...
def _generate_contract_id(label: str, page_num: int) -> str:
    ...
    if "resume" in lbl or "cv" in lbl:
        ...
        prefix = "PAN"
    elif "mark" in lbl or "10th" in lbl or "12th" in lbl or "degree" in lbl:
        prefix = "MKS"
    elif "experience" in lbl or "internship" in lbl or "relieving" in lbl:
        prefix = "EXP"
...
async def _process_single_page(
    page_num: int, page_pixmap_bytes: bytes, blueprint: Dict[str, Any]
) -> Dict[str, Any]:
    blueprint_str = json.dumps(blueprint, indent=2) if blueprint else "{}"
    ...
    try:
        raw_output = await qwen_client.vision_async(image_bytes=page_pixmap_bytes, prompt=prompt)
        parsed = _extract_json_safely(raw_output)

        if parsed and isinstance(parsed, dict):
            label = parsed.get("label", f"Document_Page_{page_num}")
            clean_id = _generate_contract_id(label, page_num)
            return {
                "id": clean_id,
                "url": None,
                "label": label,
        ...
        logger.error(f"Error on Page {page_num}: {exc}")

    label = f"Document_Page_{page_num}"
    return {
        "id": _generate_contract_id(label, page_num),
        "url": None,
        "label": label,
        "ocr_data": {"doc_quality": "Good", "doc_quality_issues": "document seems fine"},
    }


async def run_pipeline(
    file_bytes: bytes, filename: str, blueprint: Dict[str, Any] = None
) -> Dict[str, Any]:
    doc = pymupdf.open(stream=file_bytes, filetype="pdf")
    total_pages = len(doc)
    print(f"\n[PIPELINE] Ingesting {filename} ({total_pages} pages) in full parallel stream...", flush=True)

    # Pre-render pages to release PDF lock
    page_render_tasks = []
    for idx in range(total_pages):
        pix = doc[idx].get_pixmap(dpi=110)
        page_render_tasks.append((idx + 1, pix.tobytes("jpeg")))
    doc.close()

    # Parallel asynchronous dispatch
    tasks = [
        _process_single_page(p_num, img_bytes, blueprint)
        for p_num, img_bytes in page_render_tasks
    ]
    extracted_docs = await asyncio.gather(*tasks)
