...
class ClientUnlockSignatureRequest(BaseModel):
  token: str
...
@billing_router.post("/client/unlock-signature")
async def client_unlock_signature(
    payload: ClientUnlockSignatureRequest,
    client: dict = Depends(authenticate_client),
    db: Session = Depends(get_db),
):
  """Allows an authenticated client to unlock the Premium Signature & Stamp Verification
  feature using a single-use, company-specific token generated by an administrator.
  Matches the provided token against the company's signature_unlock_token column.
  Upon match:
  - sets signature_unlocked = True
  - burns the token (signature_unlock_token = None) to prevent reuse.
  """
  company_id = client.get("company_id")
  if not company_id:
    raise HTTPException(
  ...
  if company.signature_unlocked:
    return {
        ...
        "message": "Signature feature is already unlocked for this organization.",
    }

  provided_token = payload.token.strip() if payload.token else ""
  if (
      not provided_token
      or not company.signature_unlock_token
      or provided_token != company.signature_unlock_token
  ):
    raise HTTPException(
        status_code=400,
        detail="Invalid or expired signature unlock token. Please request a new token from your administrator.",
    )

  # Burn single-use token and unlock feature
  company.signature_unlocked = True
  company.signature_unlock_token = None

  try:
    storage_manager.update_company_addon(
        company_id=company_id, signature_enabled=True
    )
  except Exception:
    pass
...
@billing_router.get("/admin/billing/pricing")
async def get_billing_rates(
    admin: dict = Depends(authenticate_admin),
):
  """Admin view: Retrieve current per-page and signature verification pricing rates.
  Protected by admin authentication.
  """
  pricing = storage_manager.get_pricing()
