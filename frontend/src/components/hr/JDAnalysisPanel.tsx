...
export const JDAnalysisPanel: React.FC<JDAnalysisPanelProps> = ({
  jobTitle,
  extractedRequirements,
  rawJdText,
  onReanalyze,
  ...
  const education = extractedRequirements.education || [];
  const certifications = extractedRequirements.certifications || [];
  const responsibilities = extractedRequirements.key_responsibilities || [];
  const experienceYears = extractedRequirements.experience_years || 'Not specified';
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg text-slate-900 font-bold tracking-tight">{jobTitle || 'Active Job Specification'}</h2>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Criteria extracted and utilized by the Explainable AI Matching Engine
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          {rawJdText && (
            ...
            >
              ...
              <span>{showRawText ? 'Hide Source JD' : 'View Source JD'}</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
        ...
        <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-start gap-2.5">
          ...
          <div>
            ...
            <div className="text-xs font-bold text-slate-900 mt-0.5 truncate max-w-[170px]" title={education.join(', ')}>
              {education.length > 0 ? education.join(', ') : 'Open / Relevant Field'}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-start gap-2.5">
          ...
          <div>
            <div className="text-slate-500 uppercase text-xs font-semibold tracking-wider">Certifications</div>
            <div className="text-xs font-bold text-slate-900 mt-0.5 truncate max-w-[170px]" title={certifications.join(', ')}>
              {certifications.length > 0 ? certifications.join(', ') : 'Not required'}
            </div>
          </div>
        </div>
        ...
        <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 flex items-start gap-2.5">
          ...
          <div>
            <div className="text-slate-500 uppercase text-xs font-semibold tracking-wider">Skills Analyzed</div>
            <div className="text-xs font-bold text-slate-900 mt-0.5">{skills.length} Key Competencies</div>
          </div>
        </div>
      </div>

      {/* Core Technical & Role Skills */}
      <div className="mb-4">
      ...
      {responsibilities.length > 0 && (
        <div className="pt-3 border-t border-slate-200">
          <div className="text-xs font-semibold text-slate-700 mb-2">Key Job Responsibilities</div>
      ...
      {showRawText && rawJdText && (
        <div className="mt-4 pt-4 border-t border-slate-200">
          <div className="text-xs font-semibold text-slate-700 mb-2">Raw Job Description Source</div>
